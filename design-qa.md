...
# 智能创作工作台折叠式重设计（2026-08-08）

## 对比对象

- 设计源图：`C:\Users\zeng\.codex\generated_images\019fdf9a-f2e1-7a32-872c-5d0d719f9606\exec-897499aa-5e37-47b1-af46-33451bd1e63d.png`
- 实现截图：`C:\Users\zeng\.codex\visualizations\2026\08\08\019fdf9a-f2e1-7a32-872c-5d0d719f9606\pipeline-implementation.png`
- 并排对比图：`C:\Users\zeng\.codex\visualizations\2026\08\08\019fdf9a-f2e1-7a32-872c-5d0d719f9606\pipeline-design-comparison.png`
- 路由与状态：`/pipeline?batch=production-batch-44feaa91224a&run=pipeline-c20e059afa98`，第 2 步“确认创作方案”，云端转写结果暂时无法确认。
- 实际浏览器视口：1291 × 906 CSS px，device pixel ratio 1.5；实现截图为 1285 × 902 px。
- 源图为 1487 × 1058 px。并排图为 1440 × 552 px，按各自比例缩放后对比，没有拉伸。

## 全局对比

实现采用设计图的单一任务焦点结构：顶部三步流程 → 左侧当前操作 → 右侧素材与费用摘要 → 下方待处理任务和 IP 配方。当前步骤默认展开，已完成和后续步骤收起；信息不再以多个同权大卡片挤满首屏。

## 重点区域与交互验收

- 步骤折叠：实际页面中第 2 步默认展开，第 1、3 步默认收起；三个步骤均可展开和收起。
- 当前操作：结果未知时居中显示“转写结果待核对”，保留真实错误说明和 35% 进度；唯一主操作为“刷新实时状态”，没有出现重新提交、重试或继续付费入口。
- 素材与费用：“所选素材（1）”默认展开；“费用与提交”默认收起，并明确显示“不会自动重复扣费”。两者均已点击验证折叠状态。
- 待处理任务：默认只显示 7 条待处理任务的摘要，点击后才展示列表，避免任务较多时把首屏撑长。
- IP 配方：默认收起并显示人物、音色摘要，点击后才展示完整配置。
- 可访问性：所有折叠按钮均暴露 `aria-expanded`，键盘焦点样式可见。
- 浏览器控制台：未发现错误。

## 比较历史

- 第一次对比：[P1] 结果未知仍表现为普通信息卡，用户无法在 5 秒内识别当前最重要的问题；[P2] IP 配方默认展开，继续制造纵向噪声。
- 第一次修复：将结果未知改为居中的主状态区，突出“转写结果待核对”和安全说明；IP 配方改为默认收起的摘要行。
- 第二次对比：重新捕获实现截图并与源图并排检查，未发现 P0、P1 或 P2 级差异。
- 可接受差异：设计图使用折叠侧栏和示例文案，实际页面保留用户当前展开的侧栏、真实素材标题、真实任务数量；设计图中的“查看处理建议”在真实未知结果状态下替换为安全的“刷新实时状态”，避免诱导重复付费。

## 验收结论

视觉层级、当前步骤聚焦、默认折叠策略、窄屏响应式规则和真实错误状态均符合选定设计方向；未触发云端重试、付费处理或发布操作。

- 前端全量测试：138/138 通过。
- TypeScript 类型检查：通过。
- Vite 生产构建：通过；仅保留已有的大分包体积提醒，不影响本次页面功能。
- `git diff --check`：通过。
