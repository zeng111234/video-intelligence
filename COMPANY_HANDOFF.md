...
- **脱敏源码包**：不含 `.env` 和任何本机运行密钥，适合代码归档；
- **配置版源码包**：按交接方要求保留当前非阿里云、非 DeepSeek 配置，适合在公司电脑复现当前运行模式；该包必须通过公司认可的安全渠道传输，不能公开上传。

当前工作区是一个仍有未提交改动的开发快照：

- 分支：`feature/brand-emphasis-style`
- 当前提交：`340f533a14b8c0d538c493c4369492c4d497b8b7`
- 工作区状态：存在约 100 条未提交/未跟踪变更
...
脱敏包的配置方式：复制 `.env.example` 为本机 `.env`，只填写公司已确认的变量。配置版源码包已经保留当前非阿里云、非 DeepSeek 的本机配置，但仍应由公司管理员检查并替换为公司自己的账号。源码预览可以使用 `scripts\\start_all_services.ps1 -UseCompanyServer`，但必须先确认公司控制层地址和权限，且先用低成本/沙箱模式验收。
...
以下内容属于本机运行状态、个人数据、客户数据或敏感凭据，不应进入脱敏源码包；配置版只按本次明确交接范围保留配置，不包含阿里云和 DeepSeek 密钥：
...
配置版源码包中的剩余 `.env` 和 `.streamlit/secrets.toml` 仍可能含有其他供应商凭据，接收后应立即转入公司的密钥管理系统并按公司流程轮换。
...
| 文件 | SHA256 |
|---|---|
| `交付文件/exports/VideoInsight-company-configured-handoff-2026-09-11.zip` | `7FECBC0B44B2D4DA145AE7F1420BD6D4A0203C8B4A0C4B4F2801D4C295D44EF9` |
