...
本次只交付**公司环境变量版脱敏源码包**：包内只保留源码、锁文件、运行资源、`.env.example` 和文档，不含开发机 `.env`、`.env.production`、`.streamlit/secrets.toml`、数据库、日志、浏览器登录态或 SSH 私钥。公司已经在 Windows 系统/用户环境中配置的非空变量，启动脚本会优先读取；没有配置的变量才回退到本机 `.env`。本次不交付 Windows EXE，接收方应从源码运行和二开。

此前生成的 `VideoInsight-company-configured-handoff-2026-09-11.zip` 曾包含本机配置文件，已标记为**不可交付**；不要把它上传 GitHub 或发给客户。

当前交付的权威源是源码 ZIP；如果公司选择从 GitHub 接收，必须先核对分支、提交号和关键文件 SHA256，确认与 ZIP 一致后再使用：

- 分支：`codex/company-handoff-20260911`
- 交付提交：接收方应先记录 `git rev-parse HEAD`，并与交付人确认提交号
- 交付分支工作区：必须由接收方在当前交付时重新核对，不能沿用历史报告
- 运行资源：已显式纳入 `data/templates/builtin.json`，不依赖开发机被忽略的本地目录
- 交付包：源码 ZIP 与包外 SHA256 记录同步更新；本次不包含安装包
...
源码启动、旧 EXE 和公司预览包可能使用不同的数据目录。SQLite 数据、任务记录和抖音等平台的浏览器登录态跟随具体运行目录，不会自动跨目录共享；更换启动方式或解压目录后需要按公司账号重新登录。不要把开发机数据库或浏览器 profile 直接复制给公司。
...
本次不向公司交付 Windows EXE。`scripts/build_windows_installer.ps1` 和相关安装包脚本仅作为公司后续自行构建的开发参考；目录中残留的 0.2.48/0.2.49 安装包均不属于本次交付，不能作为源码版本或业务验收证据。

如果公司未来需要自行构建安装包，必须重新构建并记录：
...
当前源码把本机剪辑定位为“可审查草稿导出”，不是正式发布成片：

- `visual_release`、`creative`、`publish_rights` 仍会计算并写入 `quality_report`，但不再阻断本机草稿导出；
- 这些结果会记录到 `draft_export_advisory_failures`，供公司侧补充 B-roll、创意方案和素材授权证据；
- `media_integrity`、`subtitle_sync`、`subtitle_word_timing` 以及基础音视频检查仍是本机硬门禁；
- 即使本机草稿导出成功，也不会自动把 `publish_allowed` 或 `publish_claim_allowed` 设为真；
- 修改后必须重启 2001 后端，已有任务不会自动加载新代码；重启后应重新执行一次本机剪辑验证。

因此，看到“草稿导出成功”只能证明文件可供审查，不能替代公司侧的真实页面播放、授权核验、最终视觉验收和正式发布验收。
...
历史基线曾从 GitHub 的 `codex/company-handoff-20260911` 分支完成一次全新克隆，未复用原开发目录的 `.git`、`.venv`、`venv`、`node_modules`、`.env`、数据库、日志、`work/`、`outputs/`、`build/` 或浏览器状态。当前接收方仍必须重新核对源码 ZIP、分支和提交号是否一致；运行时必需的 `data/templates/builtin.json` 应作为版本文件存在。
...
- 历史上曾生成过 Windows x64 安装包，但安装包不属于本次源码交付，也不作为本次验收证据；
- 本次只要求验证源码入口 `start.bat`、1001 前端和 2001 后端。
...
`历史基线全量 Python 测试：PASS（可选历史 work/证据缺失时按规则跳过）；最新剪辑门禁改动需重启后端并重新复验`；

`正式视频剪辑 quote → user_confirm → generation → export → 页面播放 → 下载：PENDING（尚未在公司账号和公司电脑完成）`；

`公司实际电脑验收：PENDING（尚未在客户电脑完成）`。

### 21.3 GitHub 分支与源码 ZIP 一致性

公司接收时优先使用本次源码 ZIP。若改用 GitHub，必须由交付人和接收方共同确认分支、commit、关键文件 hash 与 ZIP 一致；在完成核对前，不要从其他旧分支取代码。完成公司电脑实装验收前，仍不要承诺所有网络、杀毒软件策略和供应商账号环境都无差异。
...
- [ ] 记录验收结果、日志路径、最终源码 ZIP SHA256 和负责人。

### 21.5 Windows 安装包说明

本次不交付 Windows 安装包。公司后续如需要 EXE，应从已核对的源码提交重新构建、扫描并生成新的 SHA256；历史 0.2.48/0.2.49 安装包不能代表当前源码，也不能替代源码入口验收。
