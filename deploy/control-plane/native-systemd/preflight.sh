...
if grep -Eq '^[[:space:]]*PATH[[:space:]]*=' "$VIDEOINSIGHT_ENV_FILE"; then
  die "控制层配置不得覆盖固定的 systemd 服务 PATH。"
fi
...
printf '受信任付费验收媒体工具：ffprobe=%s ffmpeg=%s\n' \
