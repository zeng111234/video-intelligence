...
function accountStatusMeta(status: string) {
  if (status === "ready") return { label: "已保存登录状态", color: "success" };
  if (status === "browser_open") return { label: "等待扫码", color: "processing" };
...
function platformHint(platform: PublishPlatformCapability) {
  if (platform.mode === "local_browser") return "在本机打开官方创作者窗口，扫码后继续发布。";
...
export default function PublishPage() {
  ...
  const [accountName, setAccountName] = useState("公司主号");
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const firstReadyAccount = accountData.find((account) => account.status === "ready");
      setAvailablePlatforms(platformData.platforms);
      setAssets(assetData.items);
      setTasks(batchData.items.flatMap((batch) => batch.tasks));
      setAccounts(accountData);
      setSelectedAccountId((current) => (
        current && accountData.some((account) => account.account_id === current && account.status === "ready")
          ? current
          : firstReadyAccount?.account_id
      ));
      if (firstReadyAccount) setPageStep("publish");
    } catch (error) {
  ...
  const addAndConnectAccount = async () => {
  ...
  const togglePlatform = (platform: PublishPlatformCapability) => {
    ...
    if (platform.requires_account && readyAccounts.length === 0) {
      setPageStep("configure");
      toast.error("请先连接并确认一个抖音账号");
      return;
    }
  ...
  const startPublishing = async () => {
    ...
    try {
      ...
      Modal.confirm({
        title: "确认创建发布任务？",
        content: `将为 ${platforms.map(platformLabel).join("、")} 创建发布任务。系统不会代替你点击平台最终发布按钮。`,
        okText: "确认创建",
        cancelText: "返回修改",
        onOk: confirmCreateBatch,
      });
    } catch (error) {
  ...
  const columns: ColumnsType<PublishResponse> = [
    ...
    { title: "当前进度", dataIndex: "stage", ellipsis: true },
    {
      title: "下一步",
      width: 180,
  ...
  return <div>
    <div style={{ marginBottom: 20 }}>
      ...
      <Text type="secondary">先准备账号，再选择成片。系统只协助打开和填写官方页面，最终发布始终由你确认。</Text>
    </div>
    ...
    <Card size="small" style={{ marginBottom: 16 }}>
      <Space wrap>
        ...
        <Text type="secondary">已准备抖音账号：{readyAccounts.length} 个</Text>
      </Space>
    </Card>
    ...
    {pageStep === "configure" ? <Spin spinning={loading}>
      <Alert
        ...
        description="抖音只需在本机官方窗口扫码；快手、小红书和视频号会在发布时提供人工发布助手。"
      ...
      <Row gutter={[16, 16]}>
        {availablePlatforms.map((platform) => <Col xs={24} md={12} key={platform.platform}>
          <Card
            ...
            extra={platform.mode === "local_browser" && readyAccounts.length > 0 ? <Tag color="success">可发布</Tag> : undefined}
          >
            <Space direction="vertical" size={12} style={{ width: "100%" }}>
              <Text type="secondary">{platformHint(platform)}</Text>
              {platform.mode === "local_browser" ? <>
                <Space.Compact style={{ width: "100%" }}>
                  <Input aria-label="抖音账号名称" value={accountName} onChange={(event) => setAccountName(event.target.value)} onPressEnter={addAndConnectAccount} />
                  <Button type="primary" icon={<UserAddOutlined />} loading={addingAccount} onClick={addAndConnectAccount}>添加并扫码</Button>
                </Space.Compact>
                {accounts.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="还没有抖音账号" /> : accounts.map((account) => {
                  ...
                  return <Card size="small" key={account.account_id} title={account.name} extra={<Tag color={status.color}>{status.label}</Tag>}>
                    <Space direction="vertical" size={8} style={{ width: "100%" }}>
                      ...
                      <Space wrap>
                        {account.status === "ready" ? <Button size="small" type="primary" icon={<CheckCircleOutlined />} onClick={() => { setSelectedAccountId(account.account_id); setPageStep("publish"); }}>用这个账号发布</Button> : <Button size="small" type="primary" loading={connecting === account.account_id} onClick={() => connectAccount(account.account_id)}>打开官方扫码窗口</Button>}
                        {account.status !== "ready" && <Button size="small" onClick={() => refreshAccount(account.account_id)}>我已扫码，检查状态</Button>}
          ...
          <Spin spinning={loading || submitting}>
            <Space direction="vertical" size={18} style={{ width: "100%" }}>
                ...
                <Row gutter={[10, 10]}>{availablePlatforms.map((platform) => {
                  ...
                  return <Col xs={24} sm={12} key={platform.platform}><Card size="small" style={{ borderColor: selected ? "#1677ff" : undefined }}>
                    <Space direction="vertical" size={6} style={{ width: "100%" }}>
                      ...
                      <Button block type={selected ? "primary" : "default"} disabled={unavailable} onClick={() => togglePlatform(platform)}>{selected ? "已选择" : platform.requires_account && readyAccounts.length === 0 ? "先连接账号" : "选择"}</Button>
                    </Space>
                  </Card></Col>;
                })}</Row>
              </div>
              ...
              {platforms.includes("douyin") && <div>
                ...
                <Select aria-label="抖音发布账号" style={{ width: "100%" }} value={selectedAccountId} placeholder="选择已准备好的抖音账号" options={readyAccounts.map((item) => ({ value: item.account_id, label: `${item.name} · 已保存登录状态` }))} onChange={setSelectedAccountId} />
                {readyAccounts.length === 0 && <Alert type="warning" showIcon message="还没有可用的抖音账号" action={<Button size="small" onClick={() => setPageStep("configure")}>去配置</Button>} style={{ marginTop: 8 }} />}
              </div>}
      ...
      <Col xs={24} xl={11}>
        <Card title={<Space><ClockCircleOutlined /> 发布任务 <Tag color="blue">{tasks.length}</Tag></Space>} extra={<Button size="small" onClick={loadData}>刷新</Button>}>
          <Alert type="info" showIcon message="最后一步由你确认" description={selectedPlatforms.length ? `已选择：${selectedPlatforms.map((platform) => platformLabel(platform.platform)).join("、")}。系统不会代替你点击平台最终发布按钮。` : "选择平台和成片后，任务会显示在这里。"} style={{ marginBottom: 16 }} />
          {loading ? <SkeletonCard rows={4} /> : tasks.length ? <Table rowKey="task_id" columns={columns} dataSource={tasks} pagination={{ pageSize: 8 }} size="middle" /> : <Empty description="还没有发布任务" />}
        </Card>
      </Col>
    </Row>}
