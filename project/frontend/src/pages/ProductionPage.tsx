...
export default function ProductionPage() {
  ...
  const closeDrawer = () => {
    setDrawerOpen(false); setDrawerStep(0); setDraftItems([]); setSourceText(""); setBatchName(""); setCreatedBatch(null); setPreflight(null); setRightsConfirmed(false);
  };

  const openNewBatch = () => {
    setDrawerStep(0); setDraftItems([]); setSourceText(""); setBatchName(""); setCreatedBatch(null); setPreflight(null); setDrawerOpen(true);
  };
  ...
  const createBatch = async () => {
    ...
    setSubmitting(true);
    try {
      const batch = await createProductionBatch({ name: batchName.trim(), profile_id: profileId, items: draftItems });
  ...
  const runPreflight = async () => {
    ...
    setSubmitting(true);
    try {
      setPreflight(await preflightProductionBatch(createdBatch.batch_id, { rightsHolder: rightsHolder.trim(), rightsConfirmed, publishPlatforms: ["douyin"], concurrency }));
  ...
  const startBatch = async () => {
    ...
    setSubmitting(true);
    try {
      await startProductionBatch(createdBatch.batch_id, { rightsHolder: rightsHolder.trim(), rightsConfirmed, publishPlatforms: ["douyin"], concurrency });
  ...
  const openScriptReview = async (batch: ProductionBatch, item: ProductionBatchItem) => {
    setReviewLoading(true); setReviewItem(item); setReviewBatchId(batch.batch_id); setReviewText("");
    try {
      const draft = await getPipelineReviewDraft(item.run_id);
      setReviewText(draft.default_text || draft.variants[0] || "");
  ...
  const approveReview = async () => {
    ...
    try {
      await reviewProductionBatchItems(reviewBatchId, { stage: "script", reviewer: "当前用户", items: [{ run_id: reviewItem.run_id, approved_text: reviewText }] });
      toast.success("文案已通过审核并继续生产"); setReviewItem(null); await refresh();
    } catch (error) { toast.error((error as Error).message || "审核失败"); } finally { setReviewLoading(false); }
  };

  const bulkReview = async (stage: "script" | "output") => {
    const eligible = selectedRows.filter(({ item }) => stage === "script" ? item.status === "awaiting_review" : item.status === "awaiting_publish");
    if (!eligible.length) { toast.warning(stage === "script" ? "请选择待文案审核的任务" : "请选择待成片复核的任务"); return; }
    setSubmitting(true);
    try {
      ...
      const results = await Promise.all([...byBatch.entries()].map(([batchId, items]) => reviewProductionBatchItems(batchId, { stage, reviewer: "当前用户", items: items.map((item) => ({ run_id: item.run_id })) })));
      const failed = results.flatMap((result) => result.results).filter((result) => !result.ok);
      if (failed.length) toast.warning(`${failed.length} 条任务未能通过，请查看任务原因`); else toast.success(stage === "script" ? "所选文案已进入后续生产" : "所选成片已标记为待发布");
  ...
  const itemColumns: ColumnsType<ProductionBatchItem> = [
    ...
    { title: "操作", key: "action", width: 190, render: (_, item) => <Space wrap>
      {item.status === "awaiting_review" && <Button size="small" type="primary" onClick={() => { const batch = batches.find((entry) => entry.items.some((entryItem) => entryItem.run_id === item.run_id)); if (batch) void openScriptReview(batch, item); }}>审核文案</Button>}
      ...
      <Link to={`/pipeline?run=${encodeURIComponent(item.run_id)}`}><Button size="small">详情</Button></Link>
    </Space> },
  ];
  ...
  return <Space direction="vertical" size="large" style={{ width: "100%", minWidth: 0 }}>
    <div className="production-page-title"><div><Title level={3} style={{ margin: 0 }}><AppstoreAddOutlined /> 批量生产队列</Title><Text type="secondary">按批次管理多条视频；真实进度、审核和失败原因都来自后台任务。</Text></div><Space><Button icon={<ReloadOutlined />} onClick={() => void refresh()} loading={loading}>刷新</Button><Button type="primary" icon={<PlusOutlined />} onClick={openNewBatch}>新建批次</Button></Space></div>
    ...
    <Card title="全局批次队列" extra={<Space wrap><Input.Search allowClear placeholder="搜索批次、配方或内容" value={queueFilter} onChange={(event) => setQueueFilter(event.target.value)} style={{ width: 250 }} /><Select allowClear placeholder="批次状态" value={statusFilter} onChange={setStatusFilter} style={{ width: 140 }} options={Object.entries(STATUS).map(([value, item]) => ({ value, label: item.label }))} /></Space>} bodyStyle={{ padding: 12 }}>
      {selectedRunIds.length > 0 && <div className="production-bulk-bar"><Text>已选择 {selectedRunIds.length} 条任务</Text><Space wrap><Button loading={submitting} onClick={() => void bulkReview("script")}>批量通过文案</Button><Button type="primary" loading={submitting} onClick={() => void bulkReview("output")}>批量通过成片</Button><Button onClick={() => setSelectedRunIds([])}>取消选择</Button></Space></div>}
    ...
    <Drawer title={createdBatch ? `执行批次：${createdBatch.name}` : "新建批量生产"} width={760} open={drawerOpen} onClose={closeDrawer} destroyOnClose>
      ...
      {drawerStep === 2 && <Space direction="vertical" size="large" style={{ width: "100%" }}><Alert type="info" showIcon message="预检不会调用生成或发布供应商" description="通过后才会进入后台队列；预检受阻项会被单独隔离。" /><Input value={rightsHolder} onChange={(event) => setRightsHolder(event.target.value)} placeholder="授权主体" maxLength={80} /><Select value={concurrency} onChange={setConcurrency} options={[1, 2, 3, 4, 5].map((value) => ({ value, label: `同时 ${value} 条` }))} style={{ width: 180 }} /><Checkbox checked={rightsConfirmed} onChange={(event) => setRightsConfirmed(event.target.checked)}>我确认拥有媒体、文案、肖像和声音处理授权</Checkbox><Space><Button loading={submitting} onClick={() => void runPreflight()}>运行预检</Button><Button type="primary" disabled={!preflight?.ready_count} loading={submitting} icon={<RocketOutlined />} onClick={() => void startBatch()}>启动 {preflight?.ready_count || 0} 条通过项</Button></Space>{preflight && <Card size="small" title={`预检结果：通过 ${preflight.ready_count} 条，受阻 ${preflight.blocked_count} 条`}><Paragraph type="secondary">预计媒体解析费用 ¥{preflight.estimated_cost_cny.toFixed(2)}；本月已用 ¥{preflight.monthly_budget_used_cny.toFixed(2)}。</Paragraph>{preflight.items.map((item) => <Alert key={item.run_id} showIcon type={item.ready ? "success" : "warning"} message={item.display_title || item.candidate_id || item.run_id} description={item.ready ? "可进入后台队列" : item.reasons.join("；")} style={{ marginBottom: 8 }} />)}</Card>}</Space>}
    </Drawer>

    <Modal open={Boolean(reviewItem)} title="审核口播文案" confirmLoading={reviewLoading} okText="通过并继续生产" onOk={() => void approveReview()} onCancel={() => setReviewItem(null)} width={720}><Paragraph type="secondary">通过后才会进入数字人和剪辑；可直接修改最终口播稿。</Paragraph><Input.TextArea value={reviewText} onChange={(event) => setReviewText(event.target.value)} autoSize={{ minRows: 12, maxRows: 20 }} /></Modal>
