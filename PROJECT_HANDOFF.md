...
### 2026-07-18 音频转文案第二页增量

- 顶部导航改为“音频转文案”，页头为“音频转文字文案”；没有已选爆火候选时只显示返回空状态，不进入上传流程。
- 页面采用原生 Streamlit 单列四步：候选摘要、音频优先授权上传、同步真实处理状态、全宽校对与导出。优先 MP3/M4A/WAV，兼容 MP4/MOV，不下载候选来源媒体。
...
- **音频转文案**：必须先选候选；音频优先上传并兼容 MP4/MOV、媒体双重校验、FFmpeg 音频提取、faster-whisper 本地识别、真实同步进度、低置信度复核、追加校对版本和确认成稿后下载。
...
- `app_pages/`：候选检索、音频转文案、任务记录三个 UI 页面。
...
最近一次验证结果：Ruff、编译与 78 个测试通过；覆盖进度回调及观察器隔离、安全错误、时长边界、低置信度复核、严格批准版本查找、SQLite 追加冲突和 Streamlit 页面门禁。8501 已加载当前工作树并完成真实浏览器导航验证。
