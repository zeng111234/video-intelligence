...
- `app_pages/`：候选检索、视频音轨转文案、任务记录三个 UI 页面。
...
最近一次验证结果：Ruff、格式、编译、diff 检查与 90 个测试通过；覆盖 ASR 评测、准确率模式、独立上传/直链入口、直链公网与大小边界、单次网络重试、进度回调及观察器隔离、安全错误、时长边界、低置信度复核、严格批准版本查找、SQLite 追加冲突和 Streamlit 页面门禁。100.37 秒真实视频完整服务链路成功；8501 已加载当前工作树。
