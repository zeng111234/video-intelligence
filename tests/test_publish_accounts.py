...
def test_account_rejects_unsupported_platform_and_duplicate_name(tmp_path: Path):
    manager = PublishAccountManager(
        registry_path=tmp_path / "accounts.json",
        profile_root=tmp_path / "profiles",
    )
    ...
    with pytest.raises(PublishAccountError, match="同名"):
        manager.create(platform="douyin", name="主号")
    with pytest.raises(PublishAccountError, match="只支持抖音"):
        manager.create(platform="kuaishou", name="快手号")
...
def test_creator_center_page_marks_account_ready(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    manager = PublishAccountManager(
        registry_path=tmp_path / "accounts.json",
        profile_root=tmp_path / "profiles",
    )
    ...
    monkeypatch.setattr(
        manager,
        "_debug_pages",
        lambda _port: [{"title": "抖音创作者中心", "url": "https://creator.douyin.com/creator-micro/home"}],
    )
