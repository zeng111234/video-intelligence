...
def _candidate() -> VideoCandidate:
    ...
    return VideoCandidate(
        video_id="worker-candidate", platform_item_id="worker-item", title="AI 获客案例",
        author_id="author", author_name="作者", platform=Platform.DOUYIN, category="企业服务",
        published_at=now, source_type=DataSource.LICENSED_PROVIDER, matched_by=["AI"],
        metrics=VideoMetricSnapshot(item_id="worker-candidate", sampled_at=now, likes=100, confidence=0.8),
...
def test_keyword_master_selects_candidate_and_creates_persisted_child_run():
    ...
    master = pipeline_service.start_keyword_auto_run(
        keyword="AI", candidate_count=1,
        profile={"avatar_id": "a", "voice_id": "v", "edit_template_id": "t"},
        rights_holder="测试公司", publish_platforms=["douyin"],
    )
    worker = PipelineWorker(
        repository=repository, pipeline_service=pipeline_service,
        commercial_search_service=SimpleNamespace(execute=lambda **_: SimpleNamespace(batch_id="search-worker")),
        avatar_service=None, video_editing_service=None, publish_service=None, template_service=None,
    )
    worker.tick_once()
...
def test_synchronously_completed_avatar_continues_without_waiting_for_next_tick():
    ...
    worker = PipelineWorker(
        repository=repository, pipeline_service=pipeline_service,
        commercial_search_service=None, avatar_service=avatar_service,
        video_editing_service=None, publish_service=None, template_service=None,
    )
    continued = []
