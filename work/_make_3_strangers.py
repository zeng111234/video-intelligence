...
策略：复用 r8 同 source media（avatar-2704b95149a3.mp4），但 3 套新字幕 + 新 review_snapshot
+ 新 visual_spec. 验证 pipeline 在不同输入下的行为。

新主题（避开烧烤店/金鱼/二手车/手机支架/广州）：
1. 餐饮（教培/SaaS 之外，限 1 个）
2. 教培
3. SaaS
"""
import json
...
for theme in THEMES:
    ...
    new_payload['created_at'] = '2026-08-27T13:05:00+08:00'
    new_payload['updated_at'] = '2026-08-27T13:05:00+08:00'
    new_payload['items'] = [copy.deepcopy(r8_item)]
    new_item_id = f"edit-item-{theme['id']}"
    new_payload['items'][0]['item_id'] = new_item_id
    new_payload['items'][0]['edit_task_id'] = None  # 清空旧 task
    new_payload['items'][0]['edit_plan'] = None
    new_payload['items'][0]['status'] = 'awaiting_output_confirmation'
    new_payload['items'][0]['title'] = theme['title']
    new_payload['items'][0]['selected_title'] = theme['title']
    new_payload['items'][0]['review_snapshot'] = {
        "confirmed": True,
    ...
    new_payload['items'][0]['review_confirmed_at'] = '2026-08-27T13:05:00+08:00'
    new_payload['items'][0]['job'] = {
        "task_id": None,
        "status": "pending",
        "progress": 0,
        "stage": "待生成",
    ...
    # 新 subtitle_segments（7 段，主题相关）
    new_payload['items'][0]['subtitle_segments'] = [
        {
            "start": s,
            "end": e,
            "text": t,
    ...
    new_payload['items'][0]['preview_subtitle_segments'] = new_payload['items'][0]['subtitle_segments']
    new_payload['items'][0]['subtitle_preview_source'] = 'reviewed'
...
for batch_id, item_id, payload in new_batches:
    ...
    print(f"inserted batch {batch_id}, item {item_id}, rowcount={cur2.rowcount}")

conn.commit()
conn.close()
print(f"\nTotal {len(new_batches)} batches written.")
for bid, iid, _ in new_batches:
    print(f"  batch_id={bid} item_id={iid}")
