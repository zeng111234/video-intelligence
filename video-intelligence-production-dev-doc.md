...
| 创意结构模板提取 | 中高 | 必做 | 多模态识别误差、模板质量评价 |
| 品牌内容适配 | 中高 | 必做 | 品牌安全、事实正确性、素材匹配 |
| 批量粗剪 | 高 | 第二阶段接厂商接口 | Timeline 映射、接口成本、素材上传 |
| 自动精剪 | 中 | 第三阶段验证厂商智能接口 | 审美主观、接口可控性、费用与版权 |
| 效果归因与模型迭代 | 中 | 试点期接入 | 样本量小、平台数据回传延迟 |

**总体结论**：以“1 名开发者 + Codex”采用 vibe coding，可以在 **5 天完成可演示原型、2 周完成第一阶段 MVP**。第一阶段只做一个赛道、授权视频导入、热点排序、音频转写、文案改写，并接入一家数字人厂商完成至少一条真实口播视频。字幕美化、背景合成、批量剪辑和精剪随后按框架增加，不自研数字人模型。
...
|---|---|---|---:|
| 镜头切分 | [PySceneDetect](https://github.com/Breakthrough/PySceneDetect) | 检测硬切、淡入淡出，输出镜头起止时间；官方仓库提供 Python API，并依赖 FFmpeg 完成切分 | 是 |
| 语音转文字 | [faster-whisper](https://github.com/SYSTRAN/faster-whisper) | 本地提取口播字幕和时间戳；支持 CPU/GPU 及 8-bit 量化，适合当前机器使用小模型 | 是 |
| 画面文字识别 | [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) | 提取标题、贴纸、商品卖点和 CTA；首期按关键帧运行 | 是，可降级 |
...
剪辑模块在第一阶段只生成标准 Timeline JSON 并通过 Schema 校验；数字人模块仍需接入一家真实 AvatarProvider。第二阶段分别实现 `AliyunRenderProvider`、`TencentRenderProvider` 或 `VolcengineRenderProvider`，避免产品逻辑被单一厂商锁定。
...
| Day 4 | 结构与改写 | 钩子/论点/证据/CTA 拆解，产品事实与品牌规则，新文案和相似风险 | 生成可审核的新口播文案 |
| Day 5 | 数字人任务 | `MockAvatarProvider`、任务状态页面和一家厂商控制台手工试制 | 跑通检索 → ASR → 改写 → 数字人任务闭环 |
...
| 周期 | 里程碑 | 核心任务 | 验收交付 |
|---|---|---|---|
| 第 1 周 | M0 检索与转写可用 | 完成 5 天原型；固定一个赛道；导入 300～1,000 条数据；完善排名；完成 10 条授权口播视频的音频提取和 ASR | Top 20 人工认可率 ≥ 70%；60 秒清晰口播校对时间 ≤ 5 分钟 |
| 第 2 周 | M1 改写与数字人可用 | 完善结构模板、事实约束、新文案、相似风险；实现一家真实 `AvatarProvider` | 至少 3 个口播模板，并完成 1～3 条真实数字人口播视频 |

真实数字人接口的验收依赖用户取得测试账号、合法形象/音色和 API 权限。若厂商尚未开通，代码先通过 Mock 验证并明确记录外部阻塞，不循环重试或改用未经授权服务。

### 第二阶段：厂商剪辑接口（首期验证后 1～2 周）
...
- 第二阶段接入厂商后再记录队列等待、渲染耗时、存储流量和单条费用。
...
- 热度模型、ASR 文案、结构化改写、数字人适配器和后续剪辑适配器的验收指标。
...
3. 拆解钩子、论点、证据、节奏和 CTA，生成新主题口播文案。
4. 完成事实、禁用词、连续重复片段和相似风险检查。
5. 通过统一 AvatarProvider 提交文本或授权音频，至少生成 1 条真实数字人口播视频。

本文档已覆盖系统架构、目录、数据模型、接口、评分公式、模板 Schema、时间线边界、原型验证与验收标准，可直接作为实现阶段的详细设计输入。

### 交付 C：技术路线图

- 5 天演示原型：检索、热点评分、ASR 文案、新文案和模拟数字人任务。
- 2 周第一阶段 MVP：一个赛道、Top 20 榜单、10 条口播转写、3 个模板和至少 1 条真实数字人视频。
- 第二阶段用 1～2 周增加字幕、背景、封面和批量剪辑接口；精剪随后按框架增加。
